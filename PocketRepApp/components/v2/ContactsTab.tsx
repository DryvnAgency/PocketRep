...
export default function ContactsTab({ contacts, error, tags, onSelect, onRetry, onBulkTag, onAddContact, onImportContacts, onFindVehicles, onDeleteTag, searchFocusKey = 0 }: { contacts:V2Contact[]|null; error:string|null; tags:V2Tag[]; onSelect:(c:V2Contact)=>void; onRetry?:()=>void; onBulkTag?:()=>void; onAddContact?:()=>void; onImportContacts?:()=>void; onFindVehicles?:()=>void; onDeleteTag?:(name:string)=>void; searchFocusKey?:number }) {
  ...
  return <View style={styles.root}>
    ...
    <View style={styles.bookMeta}>
      <Text style={styles.bookMetaCount}>{filtered.length} customer{filtered.length===1?'':'s'}</Text>
      {filter ? <Text style={styles.bookMetaFilter}>· {filter.kind==='tier'?`${filter.icon} ${filter.name}`:filter.name}</Text> : null}
      <View style={{flex:1}}/>
      {filter?.kind==='custom'&&onDeleteTag?<Pressable onPress={()=>confirmDeleteTag(filter.name)} hitSlop={8}><Text style={styles.filterDelete}>DELETE TAG</Text></Pressable>:null}
      {filter?<Pressable onPress={()=>setFilter(null)} hitSlop={8}><Text style={styles.filterClear}>CLEAR</Text></Pressable>:null}
    </View>
...
const styles=StyleSheet.create({
  ...
  searchWrap:{paddingTop:12,paddingHorizontal:14,paddingBottom:8,flexDirection:'row',alignItems:'center',gap:8},searchBox:{flex:1,flexDirection:'row',alignItems:'center',gap:8,backgroundColor:colors.surface2,borderWidth:1,borderColor:colors.ink4,borderRadius:radius.md,paddingHorizontal:12},addBtn:{width:42,height:42,borderRadius:radius.md,backgroundColor:colors.gold,alignItems:'center',justifyContent:'center'},addBtnText:{fontSize:22,fontWeight:'800',color:colors.ink,lineHeight:24},importBtn:{width:42,height:42,borderRadius:radius.md,backgroundColor:colors.surface2,borderWidth:1,borderColor:colors.goldBorder,alignItems:'center',justifyContent:'center'},importBtnText:{fontSize:19,fontWeight:'800',color:colors.gold,lineHeight:22},searchIcon:{color:colors.grey,fontSize:14},searchInput:{flex:1,color:colors.white,fontSize:16,paddingVertical:10},searchClear:{color:colors.gold,fontSize:14},
  chipsRow:{paddingHorizontal:14,paddingTop:4,paddingBottom:8,gap:7},chip:{flexDirection:'row',alignItems:'center',gap:5,paddingHorizontal:13,paddingVertical:7,borderRadius:radius.full,borderWidth:1},chipDot:{width:6,height:6,borderRadius:3},chipIcon:{fontSize:11},chipText:{fontSize:12,fontWeight:'700',letterSpacing:.3},bookMeta:{minHeight:30,marginHorizontal:14,marginBottom:4,paddingHorizontal:2,flexDirection:'row',alignItems:'center',gap:7},bookMetaCount:{fontSize:11,fontWeight:'700',color:colors.grey2},bookMetaFilter:{fontSize:11,fontWeight:'700',color:colors.gold},filterClear:{fontSize:11,fontWeight:'700',color:colors.gold},filterDelete:{fontSize:11,fontWeight:'700',color:colors.red,marginRight:12},
