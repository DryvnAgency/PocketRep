...
function CallQueue({ contacts, onClose }: { contacts: V2Contact[]; onClose: () => void }) {
  ...
  return <View style={styles.queueOverlay}>
    <View style={styles.queueCard}>
      <View style={styles.queueTop}><View><Text style={styles.queueEyebrow}>CALL QUEUE</Text><Text style={styles.queueCount}>{index + 1} of {queue.length}</Text></View><Pressable onPress={onClose} hitSlop={10}><Text style={styles.queueClose}>✕</Text></Pressable></View>
      ...
      {!called ? <Pressable onPress={openCall} style={styles.queueCall}><Text style={styles.queueCallIcon}>📞</Text><Text style={styles.queueCallText}>CALL {current.name.split(' ')[0].toUpperCase()}</Text></Pressable> : <>
        <Text style={styles.queueQuestion}>What happened?</Text>
        <View style={styles.outcomeGrid}>
          <Pressable onPress={() => chooseOutcome('answered')} style={styles.outcomeBtn}><Text style={styles.outcomeIcon}>✓</Text><Text style={styles.outcomeText}>ANSWERED</Text></Pressable>
          <Pressable onPress={() => chooseOutcome('no-answer')} style={styles.outcomeBtn}><Text style={styles.outcomeIcon}>∅</Text><Text style={styles.outcomeText}>NO ANSWER</Text></Pressable>
          <Pressable onPress={() => chooseOutcome('voicemail')} style={styles.outcomeBtn}><Text style={styles.outcomeIcon}>▣</Text><Text style={styles.outcomeText}>VOICEMAIL</Text></Pressable>
          <Pressable onPress={() => chooseOutcome('wrong-number')} style={styles.outcomeBtn}><Text style={styles.outcomeIcon}>×</Text><Text style={styles.outcomeText}>WRONG #</Text></Pressable>
        </View>
      </>}
...
export default function ContactsTab({ contacts, error, tags, onSelect, onRetry, onBulkTag, onAddContact, onImportContacts, onDeleteTag }: { contacts:V2Contact[]|null; error:string|null; tags:V2Tag[]; onSelect:(c:V2Contact)=>void; onRetry?:()=>void; onBulkTag?:()=>void; onAddContact?:()=>void; onImportContacts?:()=>void; onDeleteTag?:(name:string)=>void }) {
  ...
  return <View style={styles.root}>
    ...
    {letters.length===0?<View style={styles.empty}><Text style={styles.emptyText}>No matches</Text></View>:letters.map(L=><View key={L}><Text style={styles.letterHead}>{L}</Text><View style={styles.group}>{groups[L].map((c,i)=><View key={c.id} style={[i>0&&styles.rowDivider]}><ContactRow c={c} onTap={()=>onSelect(c)} allTags={tags}/></View>)}</View></View>)}
