...
// the fast 5/18/40s activation stagger, the once-only "responded" dedup gate,
// and idempotent registration. Runs with plain `node` — no test runner, no
// network, no PII.
//
...
function registerDemoSend(store, contactId, nurtureMessageId, index, now) {
  ...
  if (store.entries.some(e => e.nurtureMessageId === nurtureMessageId)) return store;
...
// --- stagger: 3 demo sends get 5/18/40s --------------------------------------
const t0 = 1_000_000;
let store = { entries: [] };
...
ok('stagger: offsets are 5/18/40s', store.entries.map(e => e.offsetMs).join(',') === '5000,18000,40000');

// --- index clamp: 4th recipient clamps to the last offset --------------------
registerDemoSend(store, 'c4', 'm4', 3, t0);
ok('clamp: index 3 → 40000', store.entries.find(e => e.nurtureMessageId === 'm4').offsetMs === 40_000);
...
// --- due timing: first reply is deliberately fast for the aha ----------------
ok('due: none at +4.9s', dueEntries(store, t0 + 4_900).length === 0);
ok('due: only #1 at +5s', dueEntries(store, t0 + 5_000).map(e => e.nurtureMessageId).join() === 'm1');
ok('due: #1+#2 at +18s', dueEntries(store, t0 + 18_000).map(e => e.nurtureMessageId).sort().join() === 'm1,m2');
ok('due: #1+#2+#3+#4 at +40s', dueEntries(store, t0 + 40_000).map(e => e.nurtureMessageId).sort().join() === 'm1,m2,m3,m4');
...
ok('responded gate: m1 no longer due', !dueEntries(store, t0 + 40_000).some(e => e.nurtureMessageId === 'm1'));
