...
function buildPrompt(c: V2Contact, notes: string, repName: string, dealership: string, signable: boolean): string {
...
export async function generateGamePlan(c: V2Contact, notes: string): Promise<GamePlanResult> {
  ...
  const identity = await loadRepIdentity().catch(() => ({} as { name?: string; dealership?: string }));
  ...
  const raw = await callBrain({
    maxTokens: 600,
    tier: 'flash',
    messages: [{ role: 'user', content: buildPrompt(c, notes, repName, dealership, signable) }],
  });
