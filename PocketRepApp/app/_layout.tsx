...
class ErrorBoundary extends Component<{ children: React.ReactNode }, { error: Error | null }> {
  ...
  componentDidCatch(error: Error, info: any) {
    log.error('error-boundary', error.message, { name: error.name, stack: error.stack, componentStack: info?.componentStack });
  }
  render() {
    if (this.state.error) return (
      <View style={eb.wrap}>
        <Text style={eb.icon}>⚡</Text><Text style={eb.title}>Something went wrong</Text>
        <Text style={eb.msg}>{this.state.error.message}</Text>
        <TouchableOpacity style={eb.btn} onPress={() => this.setState({ error: null })}><Text style={eb.btnText}>Try again</Text></TouchableOpacity>
      </View>
    );
...
const eb = StyleSheet.create({
  ...
  icon: { fontSize: 40, marginBottom: spacing.md }, title: { fontSize: 20, fontWeight: '700', color: colors.white, marginBottom: spacing.sm },
  msg: { fontSize: 13, color: colors.grey2, textAlign: 'center', marginBottom: spacing.xl }, btn: { backgroundColor: colors.gold, borderRadius: radius.lg, paddingHorizontal: spacing.xl, paddingVertical: spacing.md }, btnText: { color: colors.ink, fontWeight: '700', fontSize: 15 },
});
