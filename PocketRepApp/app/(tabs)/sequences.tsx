...
const TEMPLATES: Sequence[] = [
  ...
  // ── PROSPECT NURTURE ─────────────────────────────────────────────────────
  {
    ...
    industry: 'prospect',
    description: 'Urgency-appropriate follow-up for new prospects. Lean into end-of-month timing and upcoming holidays — that context is real and the rep should reference it. Adjust each message based on where you are in the month.',
    user_id: null, is_template: true, is_custom: false, created_at: '',
    sequence_steps: [
      { id: 't8s1', sequence_id: 'tpl_8', step_number: 1, delay_days: 0, channel: 'text', message_template: 'Hey {{first_name}}, great connecting with you today! I\'m going to do everything I can to make this easy on your end. What\'s the best way to keep in touch?', ai_personalize: false },
      { id: 't8s2', sequence_id: 'tpl_8', step_number: 2, delay_days: 1, channel: 'text', message_template: 'Hey {{first_name}}, just following up from yesterday. I pulled some info together based on what you were looking for — when can I walk you through it? Even 10 minutes makes a difference.', ai_personalize: false },
      { id: 't8s3', sequence_id: 'tpl_8', step_number: 3, delay_days: 2, channel: 'text', message_template: 'Hey {{first_name}}, heads up — [reference timing: end of month, upcoming holiday, or current promotion]. This is genuinely one of the better windows to move. I don\'t say that to push you, I say it because I\'d want someone to tell me. Want to lock something in?', ai_personalize: false },
      { id: 't8s4', sequence_id: 'tpl_8', step_number: 4, delay_days: 3, channel: 'text', message_template: 'Hey {{first_name}}, I know you\'re busy so I\'ll keep this short — [if end of month: we have until the end of the month to make something happen at this number / if holiday: the [holiday] deal closes [date]]. What\'s it going to take to get this done?', ai_personalize: false },
      { id: 't8s5', sequence_id: 'tpl_8', step_number: 5, delay_days: 5, channel: 'text', message_template: 'Hey {{first_name}}, still thinking about what you\'re looking for and I think I have something worth showing you. [If end of month: we\'re a few days out from month end — after that the numbers change.] When can we connect for 15 minutes?', ai_personalize: false },
      { id: 't8s6', sequence_id: 'tpl_8', step_number: 6, delay_days: 7, channel: 'call', message_template: 'One-week follow-up call. Reference your notes — show you remember exactly what they were looking for. Lead with: "I\'ve been thinking about your situation and I have a specific idea for you." Ask about their timeline and what\'s holding things up. If it\'s end of month or a holiday is coming, name it directly — real timing is fair game.', ai_personalize: false },
      { id: 't8s7', sequence_id: 'tpl_8', step_number: 7, delay_days: 10, channel: 'text', message_template: 'Hey {{first_name}}, still here. I know the timing hasn\'t been right yet — but I want to be your person when it is. What does the next couple weeks look like for you?', ai_personalize: false },
      { id: 't8s8', sequence_id: 'tpl_8', step_number: 8, delay_days: 14, channel: 'text', message_template: 'Hey {{first_name}}, last reach out for now — I respect your time too much to keep showing up uninvited. When you\'re ready, I\'m here and I\'ll make it worth it. 🤝', ai_personalize: false },
    ],
  },
  // ── STAFFING ─────────────────────────────────────────────────────────────
  {
  ...
  // ── DOOR-TO-DOOR ─────────────────────────────────────────────────────────
  {
    ...
    sequence_steps: [
      ...
      { id: 't10s3', sequence_id: 'tpl_10', step_number: 3, delay_days: 5, channel: 'text', message_template: 'Hey {{first_name}}, [if end of month / before a holiday: reference it — "we\'re wrapping up the month" or "before the [holiday] weekend"]. This is genuinely one of the better times to move on this — pricing and availability both look good right now. Happy to get you sorted out quickly if you want to connect today.', ai_personalize: false },
      ...
      { id: 't10s5', sequence_id: 'tpl_10', step_number: 5, delay_days: 21, channel: 'text', message_template: 'Hey {{first_name}}, been a few weeks — [if near end of month or upcoming holiday: "heading into [month-end/holiday] and wanted to check one more time before things shift"]. Still think this makes sense for you. Want to get it done this week?', ai_personalize: false },
    ],
  },
  // ── ROOFING ──────────────────────────────────────────────────────────────
  {
