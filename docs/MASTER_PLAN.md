...
| P2-A7 | **Scheduler honors `profiles.timezone`/`send_hour`** (carved from P2-A3) — local-time gating: deliver at each rep's send hour, with local date/Monday. Drift reconciled by P2-A4 (git scheduler now = deployed v8); remaining blocker is the gated cron switch daily→hourly | roadmap+audit | `nurture-scheduler` | `Agent` (after cron flip) + `Eduardo: ACTION` (cron) | 🔑 |
