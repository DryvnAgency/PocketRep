...
| # | Item | Source | Files / surface | Owner | Flags |
|---|---|---|---|---|---|
| **P2-R1** | **Voice onboarding rebuild** ✅ (v1, flag-off) — `RexOnboarding.tsx`: a short conversational first-run interview (name→dealership→role→Rex tone) behind `EXPO_PUBLIC_REX_ONBOARDING` (default off → the existing carousel renders unchanged). Captures ONLY consumed fields (`profiles.full_name` + repSettings dealership/title/voiceTone) — no dead-data collection; Rex speaks each line (web TTS). v2 captured "what you sell"/unit goals deferred until a field consumes them | roadmap | `RexOnboarding.tsx`, `AppShell.tsx`, `rexFeatureFlags.ts` | `Agent` | ✅ done (flag off) |
