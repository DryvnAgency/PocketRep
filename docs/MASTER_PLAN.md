...
| P2-A6 | **Streaming-usage metering hardening** (carved from P2-A2) — verify the SSE `flush()` meter doesn't under/double-count on early client disconnect | audit | `ai-proxy` | `Agent` | |
| P2-A7 | **Scheduler honors `profiles.timezone`/`send_hour`** (carved from P2-A3) — local-time gating: deliver at each rep's send hour, with local date/Monday. Needs the cron switched daily→hourly (gated) **and** the nurture-scheduler git↔deploy drift reconciled (deployed v8 carries #69's referral asks, absent from this stack) | roadmap+audit | `nurture-scheduler` | `Agent` (after cron flip) + `Eduardo: ACTION` (cron) | 🔑 |
